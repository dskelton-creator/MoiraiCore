...
def _enrich_report_recommendations(md: str, risk: dict, eng=None, actions: dict = None) -> str:
    """Replace the (often truncated) auto Recommendations table with complete,
    scenario-specific recommendation rows built from the actual risk register
    controls. If `actions` (risk_id -> concrete text) is supplied, those are used;
    otherwise a control-mapped sentence is generated. Falls back to original md."""
    import re
    try:
        register = risk.get("risk_register", [])
        priority = {}
        for r in register:
            lvl = r.get("risk_level", "Moderate")
            rid = r.get("risk_id")
            priority[rid] = "immediate" if lvl == "Critical" else "short_term"
        rows = []
        for r in register:
            rid = r.get("risk_id")
            name = r.get("threat_name", "")
            ctrls = ", ".join(c.get("control_id", "") for c in r.get("relevant_controls", []))
            if actions and rid in actions:
                rec = actions[rid]
            else:
                rec = f"Address via NIST 800-53 controls: {ctrls}."
            rows.append(f"| {priority.get(rid, 'short_term')} | {name} | {rec} |")
        rec_table = "## Recommendations\n\n| Priority | Risk | Recommendation |\n|----------|------|----------------|\n" + "\n".join(rows) + "\n"
        return re.sub(r"## Recommendations\n.*?(\n## NIST CSF Mapping)", rec_table + r"\1", md, flags=re.S)
    except Exception:
        return md
...
NODE_TYPES = {
    ...
    "action": {
        ...
        "color": "#5b9bf5",
        "config": {
            "action_type": {"type": "select", "options": ["http", "shell", "save-file", "load-file", "notify", "seo-audit", "threat-stride", "threat-risk", "threat-auto", "threat-report", "threat-bowtie", "deep-research"], "default": "http"},
    ...
    def _run_action(self, config, inputs):
        ...
        if action_type == "http":
            ...
            try:
                ...
                r = subprocess.run(
                    [sys.executable, str(seo_script), "audit", url],
        ...
        elif action_type == "threat-stride":
            system_name = config.get("system_name", "System")
            data_flows = config.get("data_flows", "")
            if not data_flows:
                return {"error": "No data_flows provided for STRIDE analysis"}
            try:
                threat_script = AGENT_OS_ROOT / "scripts" / "threat_mod_toolkit.py"
                r = subprocess.run(
                    [sys.executable, str(threat_script), "stride", system_name, data_flows],
                    capture_output=True, text=True, timeout=120,
                    cwd=str(AGENT_OS_ROOT),
                )
                result = json.loads(r.stdout)
                # Also render an Excalidraw STRIDE data-flow diagram (viewable in Diagrams).
                try:
                    renderer = AGENT_OS_ROOT / "scripts" / "threat_excalidraw.py"
                    spec = {
                        "stride": {
                            "asset_name": system_name,
                            "asset_type": config.get("asset_type", "Web Application"),
                            "description": config.get("description", ""),
                            "data_flows": json.loads(data_flows) if isinstance(data_flows, str) else data_flows,
                            "trust_boundaries": config.get("trust_boundaries", []),
                            "entry_points": config.get("entry_points", []),
                        }
                    }
                    spec_path = AGENT_OS_ROOT / "scripts" / "_stride_spec_tmp.json"
                    spec_path.write_text(json.dumps(spec))
                    rr = subprocess.run(
                        [sys.executable, str(renderer), "--spec", str(spec_path),
                         "--out", str(AGENT_OS_ROOT / "data" / "diagrams")],
                        capture_output=True, text=True, timeout=120, cwd=str(AGENT_OS_ROOT),
                    )
                    spec_path.unlink(missing_ok=True)
                    result["diagram"] = [l for l in rr.stdout.splitlines() if l.strip().startswith("wrote")]
                except Exception as de:
                    result["diagram_error"] = str(de)
                return result
            except Exception as e:
                return {"error": str(e)}

        elif action_type == "threat-risk":
            threats = config.get("threats", "")
            risk_level = config.get("risk_level", "Moderate")
            if not threats:
                return {"error": "No threats provided for risk assessment"}
            try:
                threat_script = AGENT_OS_ROOT / "scripts" / "threat_mod_toolkit.py"
                r = subprocess.run(
                    [sys.executable, str(threat_script), "risk", threats, risk_level],
                    capture_output=True, text=True, timeout=120,
                    cwd=str(AGENT_OS_ROOT),
                )
                return json.loads(r.stdout)
        ...
        elif action_type == "threat-auto":
            description = config.get("description", "")
            system_name = config.get("system_name", "")
            if not description:
                return {"error": "No description provided for auto-assess"}
            try:
                threat_script = AGENT_OS_ROOT / "scripts" / "threat_mod_toolkit.py"
                cmd = [sys.executable, str(threat_script), "auto", description]
                if system_name:
                    cmd.append(system_name)
                r = subprocess.run(
                    cmd,
                    capture_output=True, text=True, timeout=120,
                    cwd=str(AGENT_OS_ROOT),
        ...
        elif action_type == "threat-report":
            cfg = config
            assessments_dir = AGENT_OS_ROOT / "memory-vault" / "agents" / "threat" / "assessments"
            assessments_dir.mkdir(parents=True, exist_ok=True)
            try:
                threat_script = AGENT_OS_ROOT / "scripts" / "threat_mod_toolkit.py"
                renderer = AGENT_OS_ROOT / "scripts" / "threat_excalidraw.py"
                pdf_tool = AGENT_OS_ROOT / "scripts" / "report_to_pdf.py"
                diagrams_out = AGENT_OS_ROOT / "data" / "diagrams"
                diagrams_out.mkdir(parents=True, exist_ok=True)

                # lazy import of the threat engine (kept local to avoid import cost elsewhere)
                import importlib.util as _ilu
                _spec_mod = _ilu.spec_from_file_location("threat_mod_toolkit", str(threat_script))
                assert _spec_mod is not None, "could not load threat_mod_toolkit"
                tmod = _ilu.module_from_spec(_spec_mod)
                _spec_mod.loader.exec_module(tmod)
                eng = tmod.ThreatRiskEngine()

                # ── Preferred path: a spec with stride/bowtie (+ optional risk_register) ──
                spec = cfg.get("spec")
                if isinstance(spec, str):
                    try:
                        ...
                        spec = None
                if spec:
                    # 1) Render STRIDE + Bowtie diagrams (viewable in Diagrams panel)
                    rendered = []
                    try:
                        spec_path = AGENT_OS_ROOT / "scripts" / "_report_spec_tmp.json"
                        spec_path.write_text(json.dumps(spec))
                        rr = subprocess.run(
                            [sys.executable, str(renderer), "--spec", str(spec_path), "--out", str(diagrams_out)],
                            capture_output=True, text=True, timeout=180, cwd=str(AGENT_OS_ROOT),
                        )
                        spec_path.unlink(missing_ok=True)
                        rendered = [l for l in rr.stdout.splitlines() if l.strip().startswith("wrote")]
                    except Exception as de:
                        rendered = [f"diagram_error: {de}"]

                    # 2) Build the risk register (scenario-specific if provided, else engine auto)
                    risk_register = spec.get("risk_register")
                    risk = None
                    sa_name = spec.get("stride", {}).get("asset_name", spec.get("bowtie", {}).get("threat_name", "System"))
                    if risk_register:
                        try:
                            risk = eng.risk_assess(threats=risk_register, system_name=sa_name,
                                                   system_boundary=spec.get("boundary", ""), organisation="[Organisation]")
                        except Exception:
                            risk = None
                    if risk is None:
                        desc = spec.get("description") or spec.get("stride", {}).get("description", "") or json.dumps(spec)[:400]
                        risk = eng.auto_assess(desc, system_name=sa_name)

                    scope = spec.get("scope") or (
                        f"{sa_name} — " + (spec.get("stride", {}).get("description", "")[:120] or
                        spec.get("bowtie", {}).get("threat_name", "")))
                    report = eng.executive_report(assessment=risk, organisation="[Organisation]",
                                                  scope=scope, assessor="MoiraiCore Threat Engine")
                    md = report.get("full_markdown_report", "")
                    if risk_register:
                        md = _enrich_report_recommendations(md, risk, eng, actions=spec.get("recommendations"))
                    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
                    md_path = assessments_dir / f"threat-report-{stamp}.md"
                    md_path.write_text(md)
                    pdf_path = assessments_dir / f"threat-report-{stamp}.pdf"
                    # Render each produced .excalidraw to PNG and embed in the PDF (no browser/CDN).
                    png_tool = AGENT_OS_ROOT / "scripts" / "excalidraw_to_png.py"
                    diagram_imgs = []
                    for rl in rendered:
                        # rl looks like: "wrote /path/threat-stride-....excalidraw (NN elements)"
                        if not rl.startswith("wrote "):
                            continue
                        excal = rl.split(" (")[0].replace("wrote ", "").strip()
                        excal_p = Path(excal)
                        if not excal_p.exists():
                            continue
                        png_p = diagrams_out / (excal_p.stem + ".png")
                        try:
                            subprocess.run([sys.executable, str(png_tool), str(excal_p), str(png_p)],
                                           capture_output=True, text=True, timeout=120, cwd=str(AGENT_OS_ROOT))
                            if png_p.exists():
                                title = excal_p.stem.replace("-", " ").title()
                                diagram_imgs.append(f"{title}:{png_p}")
                        except Exception:
                            pass
                    pdf_cmd = [sys.executable, str(pdf_tool), str(md_path), str(pdf_path)]
                    for di in diagram_imgs:
                        pdf_cmd += ["--image", di]
                    subprocess.run(pdf_cmd, capture_output=True, text=True, timeout=120, cwd=str(AGENT_OS_ROOT))
                    return {
                        "report_type": "NIST Threat & Risk Assessment",
                        "markdown": str(md_path),
                        "pdf": str(pdf_path),
                        "diagrams": rendered,
                        "overall_risk_level": risk.get("summary", {}).get("overall_risk_level"),
                        "total_risks": risk.get("summary", {}).get("total_risks"),
                    }

                # ── Legacy path: pre-computed assessment JSON ──
                assessment_json = cfg.get("assessment", "")
                if not assessment_json:
                    return {"error": "No spec or assessment provided for report"}
                r = subprocess.run(
                    [sys.executable, str(threat_script), "report", assessment_json],
                    capture_output=True, text=True, timeout=120, cwd=str(AGENT_OS_ROOT),
                )
                result = json.loads(r.stdout)
                md = result.get("full_markdown_report", "")
                stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
                md_path = assessments_dir / f"threat-report-{stamp}.md"
                md_path.write_text(md)
                pdf_path = assessments_dir / f"threat-report-{stamp}.pdf"
                subprocess.run([sys.executable, str(pdf_tool), str(md_path), str(pdf_path)],
                               capture_output=True, text=True, timeout=120, cwd=str(AGENT_OS_ROOT))
        ...
        elif action_type == "threat-bowtie":
            threat_name = config.get("threat_name", "")
            if not threat_name:
                return {"error": "No threat_name provided for bowtie analysis"}
            try:
                threat_script = AGENT_OS_ROOT / "scripts" / "threat_mod_toolkit.py"
                r = subprocess.run(
                    [sys.executable, str(threat_script), "bowtie", threat_name],
                    capture_output=True, text=True, timeout=120,
                    cwd=str(AGENT_OS_ROOT),
                )
                result = json.loads(r.stdout)
                # Also render an Excalidraw bowtie diagram (viewable in Diagrams).
                try:
                    renderer = AGENT_OS_ROOT / "scripts" / "threat_excalidraw.py"
                    asset = config.get("asset", threat_name)
                    spec = {
                        "bowtie": {
                            "threat_name": threat_name,
                            "asset": asset,
                            "consequences": config.get("consequences", []),
                            "preventive_barriers": config.get("preventive_barriers", []),
                            "recovery_measures": config.get("recovery_measures", []),
                            "escalation_factors": config.get("escalation_factors", []),
                        }
                    }
                    spec_path = AGENT_OS_ROOT / "scripts" / "_bowtie_spec_tmp.json"
                    spec_path.write_text(json.dumps(spec))
                    rr = subprocess.run(
                        [sys.executable, str(renderer), "--spec", str(spec_path),
                         "--out", str(AGENT_OS_ROOT / "data" / "diagrams")],
